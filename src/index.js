export default {
  async fetch(request) {
    ...
    const ORIGIN_URL = 'https://github.com/rotorflight/rotorflight-firmware'
    ...
    const CORS_ALLOWED_ORIGINS = [/^rf\.6bit\.com$/]
    ...
    function parseRequest(request) {
      ...
      if (!origin || (origin !== null && !validateCors(origin))) {
      ...
      let {tag, file} = reqParts.groups
      return {
        url: `${ORIGIN_URL}/releases/download/${tag}/${file}`,
        corsOrigin: origin,
      }
    }
